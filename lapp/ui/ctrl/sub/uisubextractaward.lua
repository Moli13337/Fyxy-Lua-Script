---
--- Created by BY.
--- DateTime: 2023/10/11 11:54:03
---
------------------------------------------------------------------
local LChildWnd = LChildWnd
---@class UISubExtractAward:LChildWnd
local UISubExtractAward = LxWndClass("UISubExtractAward", LChildWnd)
------------------------------------------------------------------

--- 窗口成员变量初始化
--- 所有用到的变量都需要在此声明，初始化数值类型尽量不要使用table
------------------------------------------------------------------
function UISubExtractAward:UISubExtractAward()
...
--- 处理成员变量销毁工作
------------------------------------------------------------------
function UISubExtractAward:OnWndClose()
...
--- 处理窗口属性设置或一些条件检测
------------------------------------------------------------------
function UISubExtractAward:OnCreate()
...
--- 处理窗口数据初始化
------------------------------------------------------------------
function UISubExtractAward:OnStart()
...
function UISubExtractAward:OnClickShop()
...
--------------------------------------------延时处理------------------------------------------------
function UISubExtractAward:OnTimer(key)
...
function UISubExtractAward:CellListItem(item, itemdata, itempos)
...
--------------------------------------------格子道具------------------------------------------------
function UISubExtractAward:RefreshGridItem()
...
function UISubExtractAward:OnActivityConfigData()
	local sid = self._sid
	local activityData = gModelActivity:GetWebActivityDataById(sid)
	local data = activityData.config

	local skipTxt,shopId,callBg,costOne1,costOne2,goodsOne
	= data.skipTxt,data.shopId,data.callBg,data.costOne1,data.costOne2,data.goodsOne
	...
	if LxUiHelper.IsImgPathValid(callBg) then
		...
		self:SetWndEasyImage(self.mBg,callBg)
	end
...
function UISubExtractAward:OnClickPrizeSlot(entryId)
...
function UISubExtractAward:ListItem(list,item, itemdata, itempos)
...
function UISubExtractAward:OnClickOpenSelect(entryId)
	GF.OpenWnd("UIActPrizeSlotSelectPop",{sid = self._sid, entryId = entryId,pages = self._pages})
end
-- 打开奖励弹窗
function UISubExtractAward:OpenUIAward()
...
--刷新格子显示
function UISubExtractAward:RefreshTurnTableGrid(_itemEffectTrans)
...
function UISubExtractAward:InitMessage()
...
function UISubExtractAward:CheckIsAllSelect()
...
function UISubExtractAward:OnClickLog()
...
--------------------------------------------Tween动画-----------------------------------------------
function UISubExtractAward:PlayItemTween()
...
function UISubExtractAward:InitEvent()
...
function UISubExtractAward:RefreshShowList()
	local _itemShowTransList = self._itemShowTransList
	for i, v in pairs(_itemShowTransList) do
		local index = v.index
		local len = v.len
...
function UISubExtractAward:RefreshBtnText()
	local sid = self._sid
...
function UISubExtractAward:OnClickTaskBox()
	GF.OpenWnd("UIActLotteryTkPop",{sid = self._sid,pages = self._pages})
end
function UISubExtractAward:RefreshTaskBox()
	local _sid = self._sid
...
function UISubExtractAward:InitDate()
...
function UISubExtractAward:LayerListItem(item, itemdata, itempos)
...
function UISubExtractAward:InitCommand()
...
function UISubExtractAward:TurnRwardBg()
	local _itemEffectTransList = self._itemEffectTransList
	if not _itemEffectTransList then return end
...
function UISubExtractAward:ResetData(pb)
	local _pages = self._pages or {}
...
function UISubExtractAward:OnClickOneTen(type)
	local sid = self._sid
...
function UISubExtractAward:OnClickDetails()
...
function UISubExtractAward:GetCurItem(entry)
	local curLv = nil
...
--------------------------------------------点击事件------------------------------------------------
function UISubExtractAward:OnClickItemIcon(itemId)
...
--设置固定格子数据
function UISubExtractAward:SetGridItemData(entryId)
...
--设置自选格子数据
function UISubExtractAward:SetCustomGridItemData(item,selectEntryId,entryId)
...
function UISubExtractAward:RefreshData()
	local sid = self._sid
	if not sid then return end
...
function UISubExtractAward:InitGridPoolInfo()
...
--------------------------------------------兑换道具------------------------------------------------
function UISubExtractAward:RefreshItem()

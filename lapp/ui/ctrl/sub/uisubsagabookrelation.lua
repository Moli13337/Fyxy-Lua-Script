...
--- DateTime: 2024/3/29 14:46:02
---
------------------------------------------------------------------
local LChildWnd = LChildWnd
---@class UISubSagaBookRelation:LChildWnd
local UISubSagaBookRelation = LxWndClass("UISubSagaBookRelation", LChildWnd)
------------------------------------------------------------------

--- 窗口成员变量初始化
--- 所有用到的变量都需要在此声明，初始化数值类型尽量不要使用table
------------------------------------------------------------------
function UISubSagaBookRelation:UISubSagaBookRelation()
end
------------------------------------------------------------------
--- 窗口关闭
--- 处理成员变量销毁工作
------------------------------------------------------------------
function UISubSagaBookRelation:OnWndClose()
...
--- 处理窗口属性设置或一些条件检测
------------------------------------------------------------------
function UISubSagaBookRelation:OnCreate()
	LWnd.OnCreate(self)
	return true
end
------------------------------------------------------------------
...
--- 处理窗口数据初始化
------------------------------------------------------------------
function UISubSagaBookRelation:OnStart()
	LWnd.OnStart(self)
	self:InitUI()
	self:InitText()
	self:InitEvent()
...
function UISubSagaBookRelation:InitRelationList(needJump)
...
function UISubSagaBookRelation:OnDrawRelationCell(list, item, itemdata, itempos)
...
function UISubSagaBookRelation:ClickRelationCard(key, init)
...
function UISubSagaBookRelation:RefreshRelationListView(needJump)
...
function UISubSagaBookRelation:SetHeroRelationNameAndNum(nameTrans,relationName,numTxtTrans,heroes,relationHeroList)
...
function UISubSagaBookRelation:CreateRelationItem(trans, key, itemdata)

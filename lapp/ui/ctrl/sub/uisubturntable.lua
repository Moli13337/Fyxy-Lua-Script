...
---@class UISubTurntable:LChildWnd
local UISubTurntable = LxWndClass("UISubTurntable", LChildWnd)
------------------------------------------------------------------

--- 窗口成员变量初始化
--- 所有用到的变量都需要在此声明，初始化数值类型尽量不要使用table
------------------------------------------------------------------
function UISubTurntable:UISubTurntable()
	self._timeKey = "UISubTurntableTimeKey"									--轮换TimeKey
	self._timeOpenRwardKey = "UISubTurntableOpenRwardTimeKey"				--转盘延迟奖励界面TimeKey
	self._playItemTweenKey = "UISubTurntablePlayItemTweenKey"				--轮换TweenKey
	self._timeTurnRwardKey = "UISubTurntable_TurnRwardKey"					--转盘延迟奖励界面TimeKey
...
--- 处理成员变量销毁工作
------------------------------------------------------------------
function UISubTurntable:OnWndClose()
...
--- 处理窗口属性设置或一些条件检测
------------------------------------------------------------------
function UISubTurntable:OnCreate()
...
--- 处理窗口数据初始化
------------------------------------------------------------------
function UISubTurntable:OnStart()
...
function UISubTurntable:OnActivityConfigData()
	local sid = self._sid
	local activityData = gModelActivity:GetWebActivityDataById(sid)
	local data = activityData.config

	local guaTxt,guaRewardTxt,guaRewardTxtPos,turntableImage,skipTxt
	= data.guaTxt,data.guaRewardTxt,data.guaRewardTxtPos,data.turntableImage,data.skipTxt
	...
	if LxUiHelper.IsImgPathValid(turntableImage) then
		CS.ShowObject(self.mBg,true)
		self:SetWndEasyImage(self.mBg,turntableImage)
	end
	...
	if LxUiHelper.IsImgPathValid(guaTxt) then
		...
		if not string.isempty(pos) then
			self:SetAnchorPos(self.mGuaTxtImg, LxDataHelper.ParseVector2NotEmpty(pos))
		end
		local guaImgSize = data.guaImgSize or 1
		self.mGuaTxtImg.localScale = Vector2.New(guaImgSize, guaImgSize)
	end

	pos = data.guaNumPos
	if not string.isempty(pos) then
		self:SetAnchorPos(self.mGuaText, LxDataHelper.ParseVector2NotEmpty(pos))
	end
...
--设置自选格子数据
function UISubTurntable:SetCustomGridItemData(item,selectEntryId,itempos)
	...
	CS.ShowObject(iconBg1,true)
	local uiIconBg1 = LxUiHelper.FindImageCtrl(iconBg1)
	uiIconBg1.color = Color.New(1,1,1,1)
	...
	if haveItem then
		...
		local circleBg1,circleBg2 = gModelGeneral:GetCommonItemCircleBgRef(itemData)
		if LxUiHelper.IsImgPathValid(circleBg1) then
			self:SetWndEasyImage(iconBg1, circleBg1)
			self:SetWndEasyImage(iconBg2, circleBg2)
		end
...
function UISubTurntable:InitMessage()
	self:WndEventRecv(EventNames.ON_RED_CHANGE, function(...) self:RefreshShopRed() end)
...
--------------------------------------------兑换道具------------------------------------------------
function UISubTurntable:RefreshItem()
	...
	if _uiCellList then
		...
		_uiCellList = self:GetUIScroll("mItemScroll_UISubTurntable")
...
--------------------------------------------延时处理------------------------------------------------
function UISubTurntable:OnTimer(key)
...
function UISubTurntable:RefreshData()
	local sid = self._sid
...
--初始化池子
function UISubTurntable:GetPoolPageData()
...
function UISubTurntable:OnClickLog()
...
function UISubTurntable:InitEvent()
	self._modelEnumList = {
		-- [ModelActivity.MODEL_ACTIVITY_TYPE_66] = {ModelActivity.MAGIC_ACADEMY4,ModelActivity.MAGIC_ACADEMY5,ModelActivity.MAGIC_ACADEMY8},
	}
...
function UISubTurntable:TurnRwardBg()
...
--刷新格子数据
function UISubTurntable:TurnTableListItem(item, itemdata, itempos)
	...
	if not _itemEffectTrans then
		local indexText = self:FindWndTrans(item, "IndexBg/IndexText")
...
function UISubTurntable:ResetData(pb)
...
function UISubTurntable:RefreshTurntableItem()
	...
	for i, v in ipairs(_tablepage.entry) do
		local item = self:FindWndTrans(self.mItemList,"Item"..i)
...
--------------------------------------------点击事件------------------------------------------------
function UISubTurntable:CheckIsAllSelect()
...
function UISubTurntable:ListItem(list,item, itemdata, itempos)
...
function UISubTurntable:OnClickOneTen(num)
...
--刷新格子显示
function UISubTurntable:RefreshTurnTableGrid(_itemEffectTrans)
...
function UISubTurntable:OnClickShop()
...
function UISubTurntable:OnClickDetails()
	GF.OpenWnd("WndBandThemeTurnTableRatePop",{sid = self._sid})
end
function UISubTurntable:InitCommand()
...
-- 打开奖励弹窗
function UISubTurntable:OpenUIAward()
...
function UISubTurntable:OnClickOpenSelect(itempos)
	GF.OpenWnd("WndBandThemeCustomSelect",{sid = self._sid, itemIndex = itempos,pages = self._pages})
end
--设置固定格子数据
function UISubTurntable:SetGridItemData(itempos)
...
function UISubTurntable:RefreshBtnText()
	local sid = self._sid
	if not sid then return end
	local activityDataW = gModelActivity:GetWebActivityDataById(sid)
	local dataW = activityDataW.config
...
--------------------------------------------Tween动画-----------------------------------------------
function UISubTurntable:PlayItemTween()

---
--- Created by wzz.
--- DateTime: 2024/5/17 14:39:04
---
------------------------------------------------------------------
local LWnd = LWnd
---@class UIDraconicUpStarActive:LWnd
local UIDraconicUpStarActive = LxWndClass("UIDraconicUpStarActive", LWnd)
------------------------------------------------------------------

--- 窗口成员变量初始化
--- 所有用到的变量都需要在此声明，初始化数值类型尽量不要使用table
------------------------------------------------------------------
function UIDraconicUpStarActive:UIDraconicUpStarActive()
	self._effectKey = "1"
end
...
--- 处理成员变量销毁工作
------------------------------------------------------------------
function UIDraconicUpStarActive:OnWndClose()
...
--- 处理窗口属性设置或一些条件检测
------------------------------------------------------------------
function UIDraconicUpStarActive:OnCreate()
	LWnd.OnCreate(self)
	return true
end
...
--- 处理窗口数据初始化
------------------------------------------------------------------
function UIDraconicUpStarActive:OnStart()
	LWnd.OnStart(self)
	self:InitUI()
...
-- 创建特效
function UIDraconicUpStarActive:CreateEffect(trans, effectName, effectKey, effectSize)
...
-- 刷新界面
function UIDraconicUpStarActive:Refresh()
	param = {
		refId    = self._refId,
	...
	self:SetWndClick(self.mSkill1,
		function() GF.OpenWnd("UIDraconicUpStar", { refId = self._refId, starNum = self._starNum, tips = true }) end)

	self:RefreshSpeechAttr()
end

-- 初始化动画ui设置
function UIDraconicUpStarActive:InitTweenUI()
...
-- 初始事件
function UIDraconicUpStarActive:InitEvents()
	self:SetWndClick(self.mMask, function(...) if self._playEndEffect then self:WndClose() end end)
end

-- 龙语属性
function UIDraconicUpStarActive:RefreshSpeechAttr()
	local curAttrList = gModelDraconic:GetSpeechBaseAttr(self._refId, self._starNum)

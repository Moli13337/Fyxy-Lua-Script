...
--- DateTime: 2021/1/12 15:44:41
---
------------------------------------------------------------------
local LWnd = LWnd
---@class UIFairylandGift:LWnd
local UIFairylandGift = LxWndClass("UIFairylandGift", LWnd)

UIFairylandGift.HAVE_GIFT_NO_SEL = 0
UIFairylandGift.HAVE_CELL_AND_GIFT = 1
UIFairylandGift.HAVE_SEL_NO_GIFT = 2

UIFairylandGift.TYPE_BUY_FREE = 0
UIFairylandGift.TYPE_BUY_ITEM = 1
UIFairylandGift.TYPE_BUY_RMB = 2
------------------------------------------------------------------

--- 窗口成员变量初始化
--- 所有用到的变量都需要在此声明，初始化数值类型尽量不要使用table
------------------------------------------------------------------
function UIFairylandGift:UIFairylandGift()
	self:SetHideHurdle()
...
--- 处理成员变量销毁工作
------------------------------------------------------------------
function UIFairylandGift:OnWndClose()
	self:ClearCommonIconList(self._uiCommonList)
	self._uiCommonList = nil

	LUtil.ClearHashTable(self._uiListTbl)
	self._uiListTbl = nil
...
--- 处理窗口属性设置或一些条件检测
------------------------------------------------------------------
function UIFairylandGift:OnCreate()
	LWnd.OnCreate(self)
...
--- 处理窗口数据初始化
------------------------------------------------------------------
function UIFairylandGift:OnStart()
	LWnd.OnStart(self)
	self:InitUI()
	self:InitEvent()
	self:InitMsg()
...
function UIFairylandGift:OnDrawItemCell(list,item,itemdata,itempos)
...
function UIFairylandGift:OnActivityConfigData(data, sid)
	if sid ~= self._sid then return end
	self:SetTop()
	self:InitPageData()
end

function UIFairylandGift:InitEvent()
	self:SetWndClick(self.mReturnBtn,function() self:CloseWndFunc() end)
	self:SetWndClick(self.mAccumulateBtn,function() self:OnClickAccumulateBtn() end)
...
function UIFairylandGift:InitData()
	self._pageId = 4 			--礼包表id
	...
	self._isShowAccumulateRedPoint = false

	gModelActivity:ReqActivityConfigData(self._sid)
end

function UIFairylandGift:CreateImmobilization(trans,itemdata)
	for i = 1,3 do
...
function UIFairylandGift:BuyClick(itemdata,isSel)
...
--### Common #########################################################################################################
--####################################################################################################################
function UIFairylandGift:InitPageData()
...
--### Top ############################################################################################################
--####################################################################################################################
function UIFairylandGift:SetTop()
	...
	local giftImage = config.giftImage
	local giftTitleIcon = config.giftTitleIcon
	local giftTitlePos = config.giftTitlePos
	self:SetWndEasyImage(self.mBgImg,giftImage,nil,true)
	self:SetWndEasyImage(self.mTitleImg,giftTitleIcon,nil,true)
	self:SetAnchorPos(self.mTitleImg, LxDataHelper.ParseVector2NotEmpty(giftTitlePos))

	local showAccumulate = config.accumulateRecharge ~= 0
	CS.ShowObject(self.mAccumulateBtn, showAccumulate)
	if showAccumulate then
		local giftTabIconPos = config.giftTabIcon
		local giftTabName = config.giftTabName
		self:SetWndText(self.mAccumulateBtnText, giftTabName)
		--self:SetAnchorPos(self.mAccumulateBtn, LxDataHelper.ParseVector2NotEmpty(giftTabIconPos))
	end

	self._callGiftOptional = config.giftOptional
...
function UIFairylandGift:SetContent()
	self:InitCustomList()
end


function UIFairylandGift:OnActivityPageResp(pb,ret)
...
function UIFairylandGift:ChangeCustomList(customList,status)
	local list = {}
...
function UIFairylandGift:GetCustomList()
	local list = {}
	...
	local selList = {}
	local pageId = ModelActivity.FAIRYLAND_GIFT_CUSTOM
	...
	local shopAllList = {}
	pageId = ModelActivity.FAIRYLAND_GIFT
...
function UIFairylandGift:IsBuyNumEmpty(itemdata)
	local buyNum = itemdata.buyNum
	return buyNum > 0
end

function UIFairylandGift:CreateGift(trans,itemdata)
	...
	if showIconImg then
		...
		if itemId then
			...
			local Image = self:FindWndTrans(btn1,"Content/Image")
...
function UIFairylandGift:CreateCommonIcon(data)
...
function UIFairylandGift:GetCommonExpendType(itemdata)
...
function UIFairylandGift:RefreshRed()
	CS.ShowObject(self.mRedPoint, self._isShowAccumulateRedPoint or false)
end

function UIFairylandGift:CreateCustom(trans,itemdata)
...
function UIFairylandGift:OnDrawCustomCell(list,item,itemdata,itempos)
...
function UIFairylandGift:GetPayType(expendType,expend2)
	local txt
	local showIconImg = false
...
function UIFairylandGift:CloseWndFunc()
...
function UIFairylandGift:CommonBuyEvent(itemdata)
	...
	local buyNum = itemdata.buyNum
	local buyCountText = string.replace(ccClientText(20715), buyNum)
	local showItemList
...
function UIFairylandGift:CreateSelGiftList(trans,list,maxList, canScroll)
...
function UIFairylandGift:InitMsg()
...
function UIFairylandGift:OnActivityReceiveGoalResp(pb,ret)
...
function UIFairylandGift:OnClickAccumulateBtn()
...
--### Server #########################################################################################################
--####################################################################################################################
function UIFairylandGift:OnActivityResp(pb,ret)
	if self._sid ~= pb.sid then return end
...
function UIFairylandGift:InitCustomList()
	...
	if uiCallGiftList then
		...
		uiCallGiftList:Create(self.mCallGiftList,list,function(...) self:OnDrawCustomCell(...) end,UIItemList.WRAP)
...
function UIFairylandGift:ResetActivePageData(pb)
	...
	for i, v in ipairs(pb.pages) do
		...
		if pageId == ModelActivity.FAIRYLAND_GIFT or
			pageId == ModelActivity.FAIRYLAND_GIFT_CUSTOM then
			local pageData= gModelActivity:GenerateActivePageDataFromPb(v)
			if pageData then
				--礼包数据
				self._activityPageData[pageId] = pageData
			end
		elseif pageId == ModelActivity.FAIRYLAND_ACCUMULATE then
	...
	if accumulateData then
		...
		for k,v in ipairs(accumulateData.entry) do
			...
			if status == 1 then
				self._isShowAccumulateRedPoint = true
				break
			end
		end
	end
...
function UIFairylandGift:GetBuyBtnStrByExpend(expend2)
	local buyTxtStr, itemImgStr
	local expend2List 	= string.split(expend2, "=")
	local isItem		= #expend2List > 1
	local showRedPoint	= false

	if not expend2 or expend2 == "" or (not isItem and expend2 == "-1") then
		--免费
		buyTxtStr = ccClientText(18712)
		showRedPoint = true
	elseif isItem then
		--道具
		local itemId 		= tonumber(expend2List[2])
		local needNum 		= tonumber(expend2List[3])
		itemImgStr	 		= gModelItem:GetItemImgByRefId(itemId)
		buyTxtStr 			= needNum
	else
		--人民币
		--local rmb = gModelPay:GetRMBValueByWelfareId(tonumber(expend2))
		buyTxtStr = gModelPay:GetShowByWelfareId(tonumber(expend2)) -- string.replace(ccClientText(15601),rmb)
	end

	return buyTxtStr, itemImgStr, showRedPoint
end

------------------------------------------------------------------
return UIFairylandGift

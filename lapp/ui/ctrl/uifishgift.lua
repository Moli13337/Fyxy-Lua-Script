---
--- Created by wzz.
--- DateTime: 2024/7/22 11:20:15
---
------------------------------------------------------------------
local LWnd = LWnd
---@class UIFishGift:LWnd
local UIFishGift = LxWndClass("UIFishGift", LWnd)
------------------------------------------------------------------

--- 窗口成员变量初始化
--- 所有用到的变量都需要在此声明，初始化数值类型尽量不要使用table
------------------------------------------------------------------
function UIFishGift:UIFishGift()
	self._refList = gModelFish:GetFishGiftRefList()
	self._shopRef = gModelFish:GetFishGiftShopRef()
end
...
--- 处理成员变量销毁工作
------------------------------------------------------------------
function UIFishGift:OnWndClose()
...
-- 点击购买按钮
function UIFishGift:OnClickBtnBuy(shopId, goodsId, ref)
...
-- 初始界面化文本
function UIFishGift:InitTexts()
	self:SetWndText(self.mTxtTitle, ccLngText(self._shopRef.name))
end

-- 绘制列表item项
function UIFishGift:OnDrawListItem(list, item, data, itemPos)
...
-- 初始事件
function UIFishGift:InitEvents()

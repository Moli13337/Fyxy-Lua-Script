...
--- DateTime: 2024/4/30 11:19:33
---
------------------------------------------------------------------
local LWnd = LWnd
---@class UIGdRk:LWnd
local UIGdRk = LxWndClass("UIGdRk", LWnd)

UIGdRk.TYPE_RANK_NORNAL = 1
UIGdRk.TYPE_RANK_DREAMTRIP = 2
------------------------------------------------------------------

--- 窗口成员变量初始化
--- 所有用到的变量都需要在此声明，初始化数值类型尽量不要使用table
------------------------------------------------------------------
function UIGdRk:UIGdRk()
	self:SetHideHurdle()
...
function UIGdRk:OnStart()
	...
	if showRankType == UIGdRk.TYPE_RANK_NORNAL then
		self:RefreshNornalView()
	end
end
-------------------------------------------------设置-------------------------------------------------------------------
--设置玩家头像
function UIGdRk:SetHeadIcon(item,info,bool)
...
function UIGdRk:TabItem(list, item, itemdata, itempos)
	...
	self:SetWndClick(item, function (...) self:OnClickTab(itemdata.type, itemdata.refId) end,LSoundConst.CLICK_PAGE_COMMON)
end

function UIGdRk:GetDreamTripList(pb)
	local list = {}
...
function UIGdRk:SetHeadIconImpl(tran,itemdata)
...
function UIGdRk:InitMessage()
	...
	self:WndNetMsgRecv(LProtoIds.MilestoneResp,function (...)
		if self._showRankType ~= UIGdRk.TYPE_RANK_NORNAL then return end
...
function UIGdRk:OnActivityConfigData(data, sid)
...
function UIGdRk:RefreshNornalView()
...
function UIGdRk:RefreshRank()
	...
	if(self._rankList)then
		...
		self._rankList:Create(self.mCellScroll,cellList,function (...) self:ListItem(...) end,UIItemList.WRAP,false)
		local list = self._rankList:GetList()
...
function UIGdRk:RefreshLikeState()
...
--设置立绘
function UIGdRk:SetHeroPaint(paintTans,info,index)--index = 1 文件形象， = 2 英雄形象
...
function UIGdRk:OnClickLookCell(refId)
	gModelRank:OnMilStoneHistoryReq(refId)
end

function UIGdRk:SetCommonData()
...
function UIGdRk:InitServerRank(item,info,isThree)--isThree=1前面3，=2列表，=3自
	if isThree == 1 then
...
function UIGdRk:OnClickLike(playerId)
...
function UIGdRk:SetAwardItem(trans,data,index)--设置排行cell	index:1=其他，2=自己
...
function UIGdRk:ShowInvasionRankTime()
...
function UIGdRk:MilestoneListItem(list,item, itemdata, itempos)
...
function UIGdRk:ChangeTab(trans,bool)
	local state = bool and 0 or 1
	self:SetWndTabStatus(trans, state)
end

function UIGdRk:InitStaticData()
...
function UIGdRk:InitRankItem(item,info,isThree)--isThree=1前面3，=2列表，=3自
	...
	if self._refId == ModelRank.RANK_MELEE or self._refId == ModelRank.RANK_GUILD_RANK then
	...
	if(isThree ~= 1)then
		...
		CS.ShowObject(iconBg, not isGuildRank)
	...
	elseif _rankRefId == ModelRank.RANK_CROSSGRADING then
		...
		if iconImgStr then self:SetWndEasyImage(iconImgTrans,iconImgStr) end
		...
		self:SetWndClick(like,function () self:OnClickLike(playerId) end)
		local isLiked = gModelRank:IsLiked(self._likeType,playerId)
		local isSelf = playerId == gModelPlayer:GetPlayerId()
		self:SetWndImageGray(like,isLiked or isSelf or self._noLikeNum)
		showLink = true
	...
	if isGuildRank then
		...
		if(info.guildLevel>0)then
			...
			memberTextIconPath = "guild_icon_24"
		...
		if isThree ~= 1 then
			...
			if _rankRefId == ModelRank.RANK_MELEE then
				self:SetWndText(serverText,playerInfo._guildName)
			end
	...
	if LxUiHelper.IsImgPathValid(memberTextIconPath) then
		self:SetWndEasyImage(memberTextIcon, memberTextIconPath, nil, true)
...
function UIGdRk:SetPlayerShow(tran,itemdata)
	if(not self._bMirrorRole)then
...
function UIGdRk:OnClickGetAward(refId)
...
--#####################################################################################################################
----------------------------------------------奖励排行------------------------------------------------------------------
function UIGdRk:RefreshAwardPage(curRewardType)
	...
	if(self._awardUiList)then
		...
		self._awardUiList:Create(self.mAwardScroll,list,function (...) self:AwardRankListItem(...) end)
...
function UIGdRk:OnClickFormation(playerId)
	--GF.OpenWnd("WndFairyTaleTDOtherHero",{playerId = playerId})
end
-----------------------------------------------点击---------------------------------------------------------------------
function UIGdRk:OnClickPlayer(playerInfo,isGuildRank)
...
function UIGdRk:OpenPlayerInfo(itemdata)
	local playerInfo = itemdata.info
	if not playerInfo then
		return
	end
...
function UIGdRk:RefreshRed(index)
...
function UIGdRk:SetRankItemT2401(item,itemdata)
	local BgImage1 = self:FindWndTrans(item,"BgImage1")
	local BgImage2 = self:FindWndTrans(item,"BgImage2")
...
function UIGdRk:AwardRankListItem(list, item, itemdata, itempos)--奖励cell
	self:SetAwardItem(item,itemdata,1)
end

function UIGdRk:OnClickTab(type, refId)
	...
	self:SetAnchorPos(self.mTabScroll,Vector2.New(-223,-537))
	...
	elseif(self._awardPageTypeList[type]) then
		...
		self:SetAnchorPos(self.mTabScroll,Vector2.New(-223,-461))
...
function UIGdRk:InitRewardList(ref)
	...
	if not self._rewardList and ref.rankReward == 1 then
		...
		for i, v in pairs(rankAwardRef) do
			if rankRewardType == 0 or rankRewardType == v.type then
				local rewardData = {
					refId = v.refId,
					type  = v.type,
				}
...
function UIGdRk:InitEvent()
	...
	self:SetWndClick(self.mBtnOneKeyMilestone,function (...) self:OnClickOneKeyMilStone() end)
end

function UIGdRk:OnClickLook(playerName,callInfo)
	callInfo.playerName = playerName
	GF.OpenWndTop("UIYellSagaSpreadWin",{StructCallShowInfo = callInfo})
end

function UIGdRk:SetRankText(tran,itemdata,isSelf,rankIcon)
	local isEmpty = not itemdata.info or itemdata.info._playerId == 0
...
function UIGdRk:ClearRankScrollList()
...
function UIGdRk:SetRankItemL2401(item,itemdata,isSelf)
	local Image = self:FindWndTrans(item,"Image")
	local RankText = self:FindWndTrans(item,"RankText")
	local FlagBg = self:FindWndTrans(item,"FlagBg")
	local FlagBgFlagIcon = self:FindWndTrans(FlagBg,"FlagIcon")
...
function UIGdRk:RefreshThreeRank(list)
	local rankTrans
	for i = 1, #list do
...
function UIGdRk:SetRankItemT2600(item,itemdata)
	local BgImage1 = self:FindWndTrans(item,"BgImage1")
	local BgImage2 = self:FindWndTrans(item,"BgImage2")
	local BgImage3 = self:FindWndTrans(item,"BgImage3")
...
--#####################################################################################################################
----------------------------------------------排行榜--------------------------------------------------------------------
function UIGdRk:RefreshDreamTripRank(list)
	if not (self._rankRootTypeList[self._oldType]) then
		return
	end
	list = list or {}
	...
	for i, v in ipairs(list) do
		...
		if playerId == selfPlayerId then selfInfo = v end
		if(i > 3) then
			table.insert(cellList,v)
		else
			table.insert(threeList,v)
	...
	if rankList then
		...
		rankList:Create(self.mCellScroll,cellList,function (...) self:ListItem(...) end,UIItemList.WRAP,false)
		local uiList = rankList:GetList()
...
function UIGdRk:SetRankIcon(tran,itemdata)
	local rank = itemdata.rank or itemdata.index
	local iconStr = "public_num_3"
	local isIn = true
	if rank == 1 then
...
function UIGdRk:OnCloseWnd()--关闭界面
...
function UIGdRk:InitTabList(ref)
...
function UIGdRk:SetRankItemL2400(item,itemdata,isSelf)
	--local Image = self:FindWndTrans(item,"Image")
	local RankText = self:FindWndTrans(item,"RankText")
	local FlagBg = self:FindWndTrans(item,"FlagBg")
	--local FlagBgFlagIcon = self:FindWndTrans(FlagBg,"FlagIcon")
...
function UIGdRk:OnClickOneKeyMilStone()
...
function UIGdRk:RefreshPlayerInfo(pb)
...
function UIGdRk:SetRankItemT2400(item,itemdata)
	local BgImage1 = self:FindWndTrans(item,"BgImage1")
	local BgImage2 = self:FindWndTrans(item,"BgImage2")
	local BgImage3 = self:FindWndTrans(item,"BgImage3")
	--local BgImage3Image = self:FindWndTrans(BgImage3,"Image")
...
--设置英雄头像
function UIGdRk:SetHeroIcon(heroIcon,heroInfo,playerId)
	local heroData={
		id=heroInfo.id,
...
function UIGdRk:OpenGuildInfo(itemdata)
	local playerInfo = itemdata.info
	if not playerInfo then
		return
	end
...
function UIGdRk:SetRankItemL2600(item,itemdata,isSelf)
	--local Image = self:FindWndTrans(item,"Image")
	local RankText = self:FindWndTrans(item,"RankText")
	local FlagBg = self:FindWndTrans(item,"FlagBg")
	--local FlagBgFlagIcon = self:FindWndTrans(FlagBg,"FlagIcon")
...
function UIGdRk:ListItem(list,item, itemdata, itempos)
...
function UIGdRk:NewPage()
	...
	elseif showRankType == UIGdRk.TYPE_RANK_DREAMTRIP then
		gModelDreamTrip:OnDreamTripRankReq()
	end
end
--设置形象
function UIGdRk:SetSpine(paintTans,ref,key)
...
function UIGdRk:RefreshTimeBg(pageType)
	local _refId = self._refId
...
----------------------------------------------里程碑--------------------------------------------------------------------
function UIGdRk:RefreshMilestone()
	...
	if(self._milestoneList)then
		...
		self._milestoneList:Create(self.mMilestoneScroll,list,function (...) self:MilestoneListItem(...) end,UIItemList.WRAP,refreshList)
	end
	if index then
...
--设置物品
function UIGdRk:SetItemIcon(itemIcon,itemData,InstanceID)

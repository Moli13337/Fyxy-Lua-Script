...
------------------------------------------------------------------
local LWnd = LWnd
---@class UISagaBookNew:LWnd
local UISagaBookNew = LxWndClass("UISagaBookNew", LWnd)
...
------------------------------------------------------------------
function UISagaBookNew:UISagaBookNew()
end
------------------------------------------------------------------
--- 窗口关闭
--- 处理成员变量销毁工作
------------------------------------------------------------------
function UISagaBookNew:OnWndClose()
...
--- 处理窗口属性设置或一些条件检测
------------------------------------------------------------------
function UISagaBookNew:OnCreate()
	LWnd.OnCreate(self)
	return true
end
------------------------------------------------------------------
...
--- 处理窗口数据初始化
------------------------------------------------------------------
function UISagaBookNew:OnStart()
	LWnd.OnStart(self)
	self:InitUI()
	self:InitText()
	self:InitData()
...
function UISagaBookNew:InitData()
	self._raceType = 0
...
function UISagaBookNew:RefreshHeroBookListView(isTop)
	self:InitHeroBookList(isTop)
	self:UpdateRedPoint()
end

function UISagaBookNew:InitMsg()
	self:WndNetMsgRecv(LProtoIds.BookChangeInfoResp,function (...) self:RefreshHeroBookListView(false) end)
...
function UISagaBookNew:PlayRaceBtnAni()
...
function UISagaBookNew:InitHeroBookList(isTop,findIndex)
...
--function UISagaBookNew:InitHeroBookList(isTop)
--	local heroList = self:GetHeroBookList()
--	local uiHeroList = self._uiHeroList
--	if uiHeroList then
--		uiHeroList:RefreshList(heroList)
...
function UISagaBookNew:OnDrawHeroSpCell(list, item, itemdata, itempos)
...
function UISagaBookNew:CreateCard(trans, itemdata)
...
function UISagaBookNew:CheckRaceTypeRedPointStatus(raceType)
...
function UISagaBookNew:InitEvent()
...
function UISagaBookNew:RefreshHeroRaceTypeList()
...
function UISagaBookNew:OnClickCareerTypeFunc(refId)
	if self._careerType == refId then return end
	self._careerType = refId

	local uiCareerTypeList = self._uiCareerTypeList
	if not uiCareerTypeList then return end
	local uiList = uiCareerTypeList:GetList()
	uiList:RefreshList()
	self:RefreshHeroBookListView(true)
end

function UISagaBookNew:ClickHeroCard(itemdata,ignore)
...
function UISagaBookNew:GetHeroBookList()
	...
	for k, v in pairs(GameTable.CharacterRef) do
		...
		local ins = (curRaceType == 0 or curRaceType == raceType) and (curCareerType == 0 or curCareerType == careerType)
		if ins then
...
function UISagaBookNew:InitCareerTypeList()
	...
	if uiCareerTypeList then
		...
		uiCareerTypeList:Create(self.mCareerTypeList,list,function(...) self:OnDrawCareerTypeCell(...) end)
	end
end

function UISagaBookNew:OnDrawCareerTypeCell(list,item,itemdata,itempos)
...
function UISagaBookNew:InitEmptyList()
	local data = {
		refId = 10005,
...
function UISagaBookNew:InitHeroRaceTypeList()
	local data = {
		wndClass = self,
...
function UISagaBookNew:GetCareerTypeList()
	local list = {}
	table.insert(list,{
...
function UISagaBookNew:CreateHeroCardList(trans, list)
...
function UISagaBookNew:InitText()
...
function UISagaBookNew:InitCommonListRect()
...
function UISagaBookNew:UpdateRedPoint()

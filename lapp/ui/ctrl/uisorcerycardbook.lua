---
--- Created by BY.
--- DateTime: 2022/7/11 18:08:05
---
------------------------------------------------------------------
local LWnd = LWnd
---@class UISorceryCardBook:LWnd
local UISorceryCardBook = LxWndClass("UISorceryCardBook", LWnd)
------------------------------------------------------------------

--- 窗口成员变量初始化
--- 所有用到的变量都需要在此声明，初始化数值类型尽量不要使用table
------------------------------------------------------------------
function UISorceryCardBook:UISorceryCardBook()
	-- self._raceList = {}
	-- self._raceItemList = {}
...
--- 处理成员变量销毁工作
------------------------------------------------------------------
function UISorceryCardBook:OnWndClose()
...
--- 处理窗口属性设置或一些条件检测
------------------------------------------------------------------
function UISorceryCardBook:OnCreate()
	LWnd.OnCreate(self)
	return true
end
------------------------------------------------------------------
...
--- 处理窗口数据初始化
------------------------------------------------------------------
function UISorceryCardBook:OnStart()
	LWnd.OnStart(self)
	self:InitUI()
	self:InitEvent()
	self:InitMessage()
...
function UISorceryCardBook:InitCommand()
	-- self:SetWndText(self.mCloseText,ccClientText(19233))
	self:SetWndText(self.mTxtClose, ccClientText(20723))
	self:SetWndButtonText(self.mBtnSpeedSet,ccClientText(29559))
	self._moveLayer = self:GetWndArg("moveLayer")

	self.botBtnData = {
		{
			name = ccClientText(29572),
			childWnd = "UISubSorceryCardBook",
			onIcon = "card_btn_1",
			offIcon = "card_btn_1",
		},
		{
			name = ccClientText(29573),
			childWnd = "UISubSorceryCardGroup",
			onIcon = "card_btn_2",
			offIcon = "card_btn_2",
		},
	}
	...
	-- uiList:Create(self.mThemeSuper,list,function(...) self:RaceListItem(...) end,UIItemList.SUPER)
	-- uiList:EnableScroll(#list > 5,true)

end

function UISorceryCardBook:OnDrawTab(_, item, itemData, index)
	...
	self:SetWndTabStatus(item, self.CurSelBtn == index and 0 or 1)
	self.tabBtn[index] = item
	CS.ShowObject(redPoint, gModelRedPoint:CheckShowRedPoint(27000000) and index == 1)
	self:SetWndClick(item, function (...) self:ClickBotBtn(index) end)
end
-- function UISorceryCardBook:OnClickType()
...
-- 		typeUiList:Create(self.mTypeSuper,list,function(...) self:TypeListItem(...) end,UIItemList.SUPER)
-- 	end
-- end
-- function UISorceryCardBook:OnClickTypeItem(type)
...
-- end
function UISorceryCardBook:OnClickSpeedSet()
	GF.OpenWnd("UISorceryCardSpeedSet")
end

function UISorceryCardBook:ClickBotBtn(index)
...
function UISorceryCardBook:InitEvent()
	self:SetWndClick(self.mCloseBtn,function ()
		self:WndClose()
	end)
	self:SetWndClick(self.mBtnCollect,function ()self:OnClickCollect() end)
	self:SetWndClick(self.mBtnHelp,function ()self:OnClickHelp() end)
	-- self:SetWndClick(self.mTypeBg,function ()self:OnClickType() end)
	self:SetWndClick(self.mBtnSpeedSet,function ()self:OnClickSpeedSet() end)
	-- self:SetWndClick(self.mTypeMask,function ()self:OnClickTypeMask() end)
end
function UISorceryCardBook:OnClickHelp()
	GF.OpenWnd("UIBzTips",{refId=380})
end

-- function UISorceryCardBook:RefreshContent()
...
function UISorceryCardBook:RefreshCollector()
	...
	CS.ShowObject(self.mBtnCollect,gModelFunctionOpen:CheckIsShow(28000002))
	local ref = gModelSorceryCard:GetCollectLevelRefByRefId(refId)
	self:SetWndText(self.mCollectText,ccLngText(ref.name))
	self:InitTextSizeWithLanguage(self.mCollectText, -4)
	self:SetWndEasyImage(self.mCollectIcon,ref.icon)
end

function UISorceryCardBook:InitTabList()
	self.tabList = self:GetUIScroll("TabScroll")
	self.tabList:Create(self.mTabScroll, self.botBtnData, function(...) self:OnDrawTab(...) end)

	self:ClickBotBtn(1)
end
-- function UISorceryCardBook:RefreshTypeShow()
...
-- 	end
-- end
-- function UISorceryCardBook:RefreshData()
...
-- 		self._uiList = uiList
-- 		uiList:Create(self.mCellSuper,itemList,function(...) self:ListItem(...) end,UIItemList.SUPER)
...
-- function UISorceryCardBook:RaceListItem(list, item, itemdata, itempos)
...
-- end
-- function UISorceryCardBook:ListItem(list, item, itemdata, itempos)
-- 	local root = self:FindWndTrans(item,"Root")
...
-- end
-- function UISorceryCardBook:CardListItem(item, itemdata, itempos,layer)
...
-- 	end ,true)
-- 	self:SetWndEasyImage(image,themeRef.cardFrame,function ()
...
-- 	CS.ShowObject(redPoint,isUp)
-- end
-- function UISorceryCardBook:TypeListItem(list, item, itemdata, itempos)
...
-- function UISorceryCardBook:OnClickRace(race)
-- 	self._isMoveTab = true
-- 	self._race = race
...
-- 	self:RefreshData()
-- end
-- function UISorceryCardBook:OnClickCard(itemdata,layer,isOpenWear)
-- 	GF.OpenWnd("UISorceryCardUpLv",{refId = itemdata.refId,callMoveLayer = layer,callTheme = self._race,isOpenWear = isOpenWear})
-- end
function UISorceryCardBook:OnClickCollect()
	GF.OpenWnd("UISorceryCardCollector")
end
function UISorceryCardBook:InitMessage()
...
function UISorceryCardBook:OnTryRefreshRedPoint()
	if self.tabList then
		self.tabList:DrawAllItems()
	end
end
-- function UISorceryCardBook:OnClickTypeMask()
-- 	self._isShowType = false
-- 	CS.ShowObject(self.mTypeMask,false)
-- 	CS.ShowObject(self.mTypeListBg,false)
-- end

-- function UISorceryCardBook:GetItemListByRace(_race)
-- 	local itemList = {}
...
-- 	return itemList
-- end
-- function UISorceryCardBook:GetThemeRed(theme)
...
-- function UISorceryCardBook:GetItemGroupByRace(_race)
-- 	local itemList = {}
